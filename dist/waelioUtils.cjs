...
var _cleanResponse = cleanResponse;
...
function parseErrors(err) {
  if (err.name) {
    ...
    return lodash.map(lodash.get(err, 'errors', []), (err) =>
      err.message.replace('Path ', '').replace('`', '').replace('`', ''),
    ).join('<br>')
  }
}
var _parseErrors = parseErrors;
