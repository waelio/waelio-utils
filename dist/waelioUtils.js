(function (global, factory) {
	...
	const _cleanResponse = (response) => {
	    ...
	    return !!data ? reParseString$2(data) : response;
	};
	...
	const _formatErrors = (err) => {
	    var errors = {};
	    lodash.get(err, 'inner', []).forEach((err) => {
	        if (lodash.get(err, 'path') && lodash.get(err, 'message')) {
	...
	*/
	...
	const _parseErrors = (err) => {
	    if (err && err.name) {
	        ...
	        return lodash.map(lodash.get(err, 'errors', []), (err) => err.message.replace('Path ', '').replace('`', '').replace('`', '')).join('<br>');
	    }
	};
	...
	exports.formatErrors = _formatErrors$1;
	...
	exports.parseErrors = _parseErrors$1;
